...
署名検証を有効にする場合は `[plugins.signature] required = true` と `entries[].signature_path` を設定します。enforce mode で plugin を実行する場合は `entries[].manifest_path` と `plugins.lockfile_path`（default: `patchgate-plugin.lock`）も設定します。
