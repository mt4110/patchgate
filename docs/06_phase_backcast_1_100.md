...
## Phase81-100 実装後の逆算（2026-03-03）

本日付で Phase81-100 の最小実装を一括反映したため、残タスクは「GA硬化」に集中します。

### 残タスク（優先順）

1. plugin sandboxの強制隔離（OS/コンテナレベル）と契約テスト整備
2. plugin SDKを多言語化し、テンプレート生成コマンドを追加
3. webhook/notification 配信の再送・重複排除・順序保証を実装
4. `policy verify-v1` 判定項目の拡張（deprecated flag/互換層終了条件）
5. release workflowでの実署名（鍵管理）と標準SBOM形式への移行
6. LTSバックポート運用を半自動化（ラベル→PR生成→検証）

### ゴール逆算マイルストーン

- M1: 互換性固定（`verify-v1` をCI必須化、legacy互換を既定OFFへ）
- M2: 運用安定化（SLO達成、通知/ webhook配信信頼性の定量化）
