...
- plugin署名検証（legacy artifact signature または signed manifest + ed25519）
- enforce mode plugin trust（signed manifest + `patchgate-plugin.lock` がない plugin は実行前に拒否）
...
- v1 LTS対象修正は `backport/lts-v1`、v2 LTS対象修正は `backport/lts-v2` ラベルで管理
